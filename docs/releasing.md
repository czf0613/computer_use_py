...
以及 ARM64 的 3.14、3.14t，使用合成媒体和离线输入测试。普通 x64 Python 额外验证
MCP；ARM64 的 cryptography wheel 和 Windows free-threaded 的 pywin32 wheel 缺失
不影响基础库发布，也不通过构建 OpenSSL 绕过。
...
1. 修改 `pyproject.toml` 中的版本号并同步 `uv.lock`；先确认目标版本尚未在 PyPI 发布。
...
macOS 15+ arm64 仅限定官方 wheel 的发行范围。源码构建默认部署目标为 macOS 13.0，
